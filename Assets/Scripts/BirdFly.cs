...
public class BirdFly : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (gameOver == false)
        {
            transform.eulerAngles =
               new Vector3(0, 0, rb.velocity.y * 20f);
        }
    ...
    private void PlayWingsFlappSound()
    {
        ...
        AudioSource.PlayClipAtPoint(wingsFlappSound,
            Camera.main.transform.position, wingsFlappSoundVolume);
    }

    private void PlayFlyUpSound()
    {
        if (flyUpSound == null) { return; }
        AudioSource.PlayClipAtPoint(flyUpSound,
            Camera.main.transform.position, flyUpSoundVolume);
    }
